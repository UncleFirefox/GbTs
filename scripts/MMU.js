var MMU = (function () {
    function MMU(cpu, gpu) {
        ...
        this.wram = new Array(8192);
        this.eram = new Array(32768);
        this.zram = new Array(128);
        this.ie = 0;
        this.if = 0;
        // MBC states
    ...
    MMU.prototype.listenForFiles = function () {
        var _this = this;
        document.getElementById("rom-selector").addEventListener("change", function (e) {
            _this.loadRom(e.target.files[0]);
        }, false);
    };
    MMU.prototype.loadRom = function (rom) {
        var _this = this;
        var reader = new FileReader();
        reader.onload = function (file) {
            var contents = file.target;
            _this.rom = contents.result;
            _this.carttype = _this.rom.charCodeAt(0x0147);
        };
        reader.readAsBinaryString(rom);
    };
    MMU.prototype.readLocalFile = function (filePath) {
        var _this = this;
        ...
        fileRequest.onreadystatechange = function () {
            if (fileRequest.readyState === 4) {
                if (fileRequest.status === 200 || fileRequest.status == 0) {
                    _this.rom = fileRequest.response;
                    _this.carttype = _this.rom.charCodeAt(0x0147);
                }
            }
        };
    ...
    MMU.prototype.writeWord = function (address, value) {
